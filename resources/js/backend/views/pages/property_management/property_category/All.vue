                ...
                >
                    <div>
                        <h4 class="bn" style="font-size: 22px !important">
                            All Property Category
                        </h4>
                    </div>
                    <div class="d-flex justify-content-between">
                        <div class="ps-3 d-flex gap-2">
                            <router-link :to="{ name: 'propertyCategoryCreate' }" class="router-link-active btn btn-info btn-sm">
                                Create  
                            </router-link>
                        </div>
                    </div>
                        ...
                        >
                            ...
                            <tbody>
                                <tr v-for="item in data?.data" :key="item.id">
                                    ...
                                    <td class="">
                                        {{ getShortTitle(item.user_creator?.name) }}
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex justify-content-end gap-1" >
                                            ...
                                            <a href="#"
                                                @click.prevent="delet_data(item.id)"
...
export default {
    ...
    methods: {
        delet_data: async function (id) {
            ...
            if (con) {
                await axios.post("/api/v1/property/category/destroy", { id: id });
                alert("Deleted");
                this.get_all_data();
            }
        },
        get_all_data: async function (url = null) {
            ...
            try {
                if (url) {
                    ...
                    res = await axios.get("/api/v1/property/category/all");
                }
