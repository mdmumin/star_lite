    ...
    <div>
        <div class="card rounded-none">
            <div class="card-header pt-3 pb-1 d-flex justify-content-between align-items-center">
                <div><h4 class="bn" style="font-size: 22px !important;">Blog tag Blog</h4></div>
                    ...
                    <table class="table table-bordered table-hover text-center">
                        <thead>
                            <tr>
                                ...
                                <th class="" style="width: 200px;">Blog Tag</th>
                        ...
                        <tbody>
                            <tr v-for="item in data" ::key="item.id">
                                ...
                                <td class="">{{ item.blog_tag.title }}</td>
                                <td><p class="badge badge-success">active</p></td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <router-link :to="{ name: 'blogTagBlogEdit', params: { id: item.id } }" class="btn btn-sm btn-outline-info">
                                         Edit 
                                         </router-link>
                                         <router-link :to="{ name: 'blogTagBlogShow', params: { id: item.id } }" class="btn btn-sm btn-outline-success">
                                         Show
                                         </router-link>
...
export default {
    ...
    methods: {
        delet_data: async function(id){
            ...
            if(con){
                await axios.post('/api/v1/blog/tag/blog/destroy', {id: id});
                alert('Deleted');
                this.get_all_data();
            }
        },
        get_all_data: async function(){
            let res = await axios.get('/api/v1/blog/tag/blog/all');
