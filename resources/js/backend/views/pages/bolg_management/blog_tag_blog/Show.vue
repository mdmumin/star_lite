                ...
                <div v-if="blogCategory">
                    <table class="table table-bordered">
                        <tbody>
                            ...
                            <tr>
                                <th style="width: 200px;">Blog tag Title</th>
                                <td>{{ blogCategory?.blog_tag?.title }}</td>
                            </tr>                        
                            <tr>
                                <th>Status</th>
                                <td>
...
export default {
    ...
    methods: {
        getBlogCategoryDetails: async function() {
            try {
                let response = await axios.get(`/api/v1/blog/tag/blog/${this.$route.params.id}`);
