                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <div class="form-group">
                                <label for="status">Blog tag ID</label>
                                <div class="mt-1 mb-3">
                                    <select class="form-control"  name="blog_tag_id">
                                    <option value="">Selet-- Blog</option>
                                    <option v-for="item in tag" :key="item.id" 
                                        :value=" item.id " 
                                        :selected="form.blog_tag_id == item.id">
                                            {{ item.title }}
                                    </option>    
                                </select>
                                </div>
    ...
    methods: {
        ...
        from_submit: async function(){
            try {
                ...
                await axios.post('/api/v1/blog/tag/blog/update', from_data);
        ...
        get_data: async function () {
            try {
                let res = await axios.get('/api/v1/blog/tag/blog/' + this.$route.params.id);
