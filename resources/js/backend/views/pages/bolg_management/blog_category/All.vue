    ...
    <div>
        <div class="card rounded-none">
            <div class="card-header pt-3 pb-1 d-flex justify-content-between align-items-center">
                <div><h4 class="bn" style="font-size: 22px !important;">All Blog Category</h4></div>
                <div class="d-flex justify-content-between">
                    <div class="ps-3 d-flex gap-2">
                        <router-link :to="{ name: 'blogCategoryCreate' }" class="router-link-active btn btn-info btn-sm">Back</router-link>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table table-bordered table-hover text-center">
                        ...
                        <tbody>
                            <tr v-for="item in data?.data" ::key="item.id">
                                ...
                                <td class="">{{ getShortTitle(item.title) }}</td>
                                <td>
                                    <img :src="getImageUrl(item.image)" alt="Cover Image" class="img-thumbnail" style="max-width: 100px;">
                                </td> 
                                <td><p class="badge badge-success">active</p></td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-3">
                                        <router-link :to="{ name: 'blogCategoryEdit', params: { id: item.id } }" class="btn btn-sm btn-outline-info">
                                         Edit 
                                         </router-link>
                                         <router-link :to="{ name: 'blogCategoryShow', params: { id: item.id } }" class="btn btn-sm btn-outline-success">
                                         Show
                                         </router-link>
...
export default {
    ...
    methods: {
        delet_data: async function(id){
            ...
            if(con){
                await axios.post('/api/v1/blog/category/destroy', {id: id});
                alert('Deleted');
                this.get_all_data();
            }
        },
        ...
        get_all_data: async function (url = null) {
            ...
            try {
                if (url) {
                    ...
                    res = await axios.get("/api/v1/blog/category/all");
                }
